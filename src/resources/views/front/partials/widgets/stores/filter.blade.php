<div class="col-sm-4 filter">
    <form method="post" action="{{ route('stores_multi_filter') }}" class="filter-multi">
                ...
                <div class="menu" tabindex="-1">
                    @foreach ($filter_locations as $location)
                        <div class="item" data-value="{{ $location->id }}">{{ $location->name }}
                            ...
                            <span>({{ $location->stores_count }})</span>
                        </div>
                    @endforeach
                </div>
            </div>
            ...
            <div class="ui multiple selection dropdown" data-filter="brand" tabindex="0" id="dropdown-brand">
                ...
                <div class="menu" tabindex="-1">
                    @foreach ($manufacturers as $manufacturer)
                        <div class="item" data-value="{{ $manufacturer->id }}">{{ $manufacturer->name }}
        ...
        <a href="{{ route('front_stores') }}"
            class="clear-filter"><span>x</span>{{ trans('messages.clear_filter') }}</a>
