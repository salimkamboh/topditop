...
class MergeCategories extends Migration
{
    ...
    public function up()
    {
		...
		foreach ($category_mapping_strings as $index => $item ) {
			//$old_cat_ID = DB::select('select id from categories where name = :name', ['name' => $item['old_cat_name']]);
			$old_cat_ID = DB::table('categories')->where(array('name'=> $item['old_cat_name'], ))->whereNotIn('id', $new_categories_ids)->first();

			//$new_cat_ID = DB::select('select id from categories where name = :name', ['name' => $item['new_cat_name']]);
		...
		//delete old category items
		//DB::table('categories')->whereIn('id', $ids_to_delete)->delete(); 
        DB::table('categories')->whereNotIn('id', $new_categories_ids)->delete();
		...
		//create frontend user to checkout display there
    }
