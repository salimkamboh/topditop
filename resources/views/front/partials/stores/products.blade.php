<section id="home-references">
    <div class="container">
        <div class="row">
            ...
            <div class="col-sm-4">
                <div class="sortable">
                    <button type="button" class="pull-left show-home-newest-prod">{{ trans('messages.newest') }}</button>
                    <button type="button" class="pull-right show-home-most-prod active">{{ trans('messages.most_viewed') }}</button>
                </div>
            </div>
        </div>
        ...
        <div class="row products-home-newest home-design hidden-block">
            @if(isset($products_newest))
                @foreach($products_newest as $product)
                    <div class="col-md-4">
                        <div class="single-item single-item-product item-shadow" data-product-id="{{$product->id}}">
                            ...
                            <div class="item-info">
                                ...
                                <div class="item-info-bottom">
                                    <a href="{{route("front_show_store", $product->store)}}"><i class="icon-shopping-cart"></i>{{$product->store->store_name}}</a>
                                    <a class="product-no-link-brand" href="#"><i class="fa fa-bookmark"></i>{{$product->manufacturer->name}}</a>
